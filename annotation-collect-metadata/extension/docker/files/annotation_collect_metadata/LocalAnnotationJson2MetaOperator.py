...
class LocalAnnotationJson2MetaOperator(KaapanaPythonBaseOperator):
    """
    ...
    * If successful, the given JSON data is included in OpenSearch
    ...
    def push_json(self, json_dict):
        print("# Pushing JSON ...")
        try:
            reference_ids = json_dict.get("99999999 ReferencedSeriesIDs_keyword")
            if reference_ids is None:
                print("# No reference ID found! - exit")
                exit(1)

            for id in reference_ids:
                json_dict = self.produce_inserts(json_dict,ref_id=id)
    ...
    def produce_inserts(self, annotation_json: dict, ref_id: str):
        print("INFO: Get reference json from index.")
        try:
            response = HelperOpensearch.os_client.get(
                index=HelperOpensearch.index, id=ref_id
            )
            series_json = response["_source"]
            print("Series found in OS")
            # add report json
            if series_json:
                print("Series_data:", series_json)
                reports = series_json.get("Reports")
                if reports:
                    # report already exits?
                    report_id = annotation_json.get("0020000E SeriesInstanceUID_keyword")
                    old_report_ids = [
                        old_report.get("0020000E SeriesInstanceUID_keyword")
                        for old_report in reports
                    ]
                    if report_id not in old_report_ids:
                        reports.append(annotation_json)
                    else:
                        print("Report already exits in Index, updated!")
                        old_report_id = old_report_ids.index(report_id)
                        reports.pop(old_report_id)
                        reports.append(annotation_json)

                else:
                    series_json["Reports"] = [annotation_json]
    ...
    def start(self, ds, **kwargs):
        ...
        self.ti = kwargs["ti"]
        print("# Starting module annotationJson2Meta")
